...
export const Market = () => {
  ...
  return (
    <div className="flex flex-col h-full space-y-8">
          ...
          factions.map(faction => (
            <div key={faction.id} className="bg-surface-container border border-outline-variant flex flex-col h-full">
              <div className="bg-surface-highest p-4 border-b border-outline-variant flex justify-between items-center">
                <h2 className="font-display text-lg uppercase tracking-widest text-primary">{faction.name}</h2>
              </div>
              <div className="p-6 space-y-4 font-mono text-sm flex-1">
                <div className="flex justify-between items-end border-b border-outline-variant/30 pb-2">
                  <span className="text-on-surface-variant text-[10px] uppercase">Preferred Tier</span>
                  <span className="text-on-surface font-bold capitalize">{faction.tier_preference}</span>
                </div>
                <div className="flex justify-between items-end border-b border-outline-variant/30 pb-2">
                  <span className="text-on-surface-variant text-[10px] uppercase">Price Ceiling</span>
                  <span className="text-primary">${faction.price_ceiling.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-end border-b border-outline-variant/30 pb-2">
                  <span className="text-on-surface-variant text-[10px] uppercase">Target Volume</span>
                  <span className="text-on-surface">{faction.volume.toLocaleString()} units</span>
                </div>
                <div className="flex justify-between items-end border-b border-outline-variant/30 pb-2">
                  <span className="text-on-surface-variant text-[10px] uppercase">Min Brand Score</span>
                  <span className="text-on-surface">{faction.brand_min > 0 ? faction.brand_min : 'None'}</span>
                </div>
                <div className="flex justify-between items-end pb-2">
                  <span className="text-on-surface-variant text-[10px] uppercase">Flexibility</span>
      ...
      <div className="bg-surface-low border border-outline-variant p-4 font-mono text-xs text-on-surface-variant mt-auto">
